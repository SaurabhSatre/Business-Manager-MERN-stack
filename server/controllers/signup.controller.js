...
const registerUserController =  async (req, res) => {
    ...
    if(!email || !name || !password || !userStatus || !validateEmail(email) || !validatePassword(password)){
        res.json({ success: false, status: 400, message : "Data is not correct!!" });
        return;
    }

    if(userStatus != "Employee" && userStatus != "Customer"){
        return res.json({ success: false, status: 400, message: "User Status is not correct!!" });
    }
